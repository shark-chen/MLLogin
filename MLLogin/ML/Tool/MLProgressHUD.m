...
#define MLMainThreadAssert() NSAssert([NSThread isMainThread], @"MLProgressHUD needs to be accessed on the main thread.");

CGFloat const MLProgressMaxOffset = 1000000.f;

static const CGFloat MLDefaultPadding = 4.f;
static const CGFloat MLDefaultLabelFontSize = 16.f;
static const CGFloat MLDefaultDetailsLabelFontSize = 12.f;
...
@interface MLProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MLProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MLProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MLProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MLProgressHUD *hud = (MLProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MLProgressHUDAnimationFade;
    _mode = MLProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MLProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MLProgressHUDAnimationZoom) {
        type = animatingIn ? MLProgressHUDAnimationZoomIn : MLProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MLProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MLProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MLProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MLProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MLProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MLProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MLBackgroundView *backgroundView = [[MLBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MLProgressHUDBackgroundStyleSolidColor;
    ...
    MLBackgroundView *bezelView = [MLBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MLDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MLDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [MLProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MLDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MLRoundProgressView class]];

    MLProgressHUDMode mode = self.mode;
    if (mode == MLProgressHUDModeIndeterminate) {
    ...
    else if (mode == MLProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MLBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MLProgressHUDModeDeterminate || mode == MLProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MLRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MLProgressHUDModeAnnularDeterminate) {
            [(MLRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == MLProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MLProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MLProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MLProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MLRoundProgressView class]]) {
        MLRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MLRoundProgressView appearanceWhenContainedIn:[MLProgressHUD class], nil];
#else
        appearance = [MLRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MLProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MLRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MLRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MLBarProgressView class]]) {
        MLBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MLBarProgressView appearanceWhenContainedIn:[MLProgressHUD class], nil];
#else
        appearance = [MLBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MLProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MLBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MLBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    MLBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MLDefaultPadding : 0.f;
...
- (void)setMode:(MLProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MLProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MLRoundProgressView
...
@implementation MLBarProgressView
...
@implementation MLBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MLProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(MLProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    MLProgressHUDBackgroundStyle style = self.style;
    if (style == MLProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MLProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation MLProgressHUDRoundedButton
